#!/usr/bin/env python3
"""
Generate model responses on Modal with A100 GPUs for fast inference.
Downloads responses locally for judge evaluation.

Usage:
    modal run evaluation/generate_responses_modal.py --config evaluation/configs/em_conservative_calibration.yaml
"""
...
# Define Modal app
app = modal.App("alignrot-evaluation")
...
@app.function(
    ...
    timeout=10800,  # 3 hours - enough for sequential generation
...
def generate_model_responses(
    ...
    for prompt_record in prompts:
        ...
        for sample_idx in range(num_samples):
            count += 1
            print(f"[{count}/{total}] Generating: {question_id}, sample {sample_idx}")
...
def main(config: str = "evaluation/configs/em_conservative_calibration.yaml"):
    """
    Main entrypoint: Load config, generate responses, save locally.
    ...
    print(f"\n{'='*70}")
    print(f"MODAL RESPONSE GENERATION")
    ...
    for model_cfg in models:
        ...
        # Generate EM responses (if any)
        if len(em_prompts) > 0:
            print(f"\nGenerating EM responses...")
            em_responses = generate_model_responses.remote(
                model_config=model_cfg,
                prompts=em_prompts,
                num_samples=em_samples,
                temperature=cfg['evaluation']['model_temperature'],
                max_tokens=cfg['evaluation']['model_max_tokens'],
            )
            all_responses.extend(em_responses)
            print(f"✓ Completed EM: {model_cfg['name']} ({len(em_responses)} responses)")

            # Save EM responses
            with open(output_file, 'a') as f:
                for response in em_responses:
                    f.write(json.dumps(response) + '\n')
            print(f"✓ Saved EM responses to: {output_file}")

        # Generate political responses (if any)
        if len(political_prompts) > 0:
            print(f"\nGenerating political responses...")
            political_responses = generate_model_responses.remote(
                model_config=model_cfg,
            ...
            all_responses.extend(political_responses)
            print(f"✓ Completed political: {model_cfg['name']} ({len(political_responses)} responses)")

            # Save political responses
            with open(output_file, 'a') as f:
                for response in political_responses:
                    f.write(json.dumps(response) + '\n')
            print(f"✓ Saved political responses to: {output_file}")

        print(f"✓ Total for {model_cfg['name']}: {len(em_prompts) * em_samples + len(political_prompts) * political_samples} responses")
...
if __name__ == "__main__":
    print("Please run with: modal run evaluation/generate_responses_modal.py --config <config_path>")
